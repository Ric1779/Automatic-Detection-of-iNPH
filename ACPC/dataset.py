...
class MedicalSegmentationDecathlon(Dataset):
    ...
    def __init__(self, task_number, dir_path, split_ratios = [0.8, 0.1, 0.1], transforms = None, mode = None) -> None:
        ...
        #Extracting .tar file
        if not os.path.exists(os.path.join(dir_path, "Datasets", self.file_name)):
            ExtractTar(os.path.join(dir_path, f"{self.file_name}.tar"))
        #Path to extracted dataset
        self.dir = os.path.join(dir_path, "Datasets", self.file_name)
...
def get_train_val_test_Dataloaders(train_transforms, val_transforms, test_transforms):
    ...
    dataset = MedicalSegmentationDecathlon(task_number=DatasetParams.TASK_ID, dir_path=DatasetParams.DATASET_PATH, transforms=[train_transforms, val_transforms, test_transforms])
